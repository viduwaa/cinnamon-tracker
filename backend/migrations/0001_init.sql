...
--
-- auth.uid() shim: reads the request-scoped setting
-- 'request.jwt.uid'. The backend connects as the table owner
-- (which bypasses RLS) and enforces visibility in its queries;
-- RLS remains defense-in-depth for any non-owner role and for a
-- future PostgREST exposure. Setting the GUC per-request makes
-- the policies live without code changes.
-- ============================================================
